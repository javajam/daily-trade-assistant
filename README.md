...
`config/ema9_trend.example.yaml` is the **noon day-trade** book on **AAPL / MSFT** (not TSLA/MU). Default product is **long-only**: EMA9 reclaim + SMA20 / RSI14 < 70, **lock-+1%**, no take, `entry_cutoff` 12:00, `flatten_by` 15:55. **Shorts are parked** — `ema9_trend_short` is not in the default enabled rules. Re-enable from `config/ema9_trend_short_optional.example.yaml` (risk / 5m siblings: `config/ema9_trend_risk_short_optional.example.yaml`, `config/ema9_trend_5m_short_optional.example.yaml`, `config/ema9_trend_risk_5m_short_optional.example.yaml`). SOXL is optional (`config/ema9_trend_bracket_soxl.example.yaml`, `config/ema9_trend_risk_soxl.example.yaml`).
...
2. **Short entry (parked)** — optional `ema9_trend_short`: price crosses **below EMA(9)** AND close **< SMA(20)**. **No RSI** on shorts. Same fill (next bar open).
...
4. **Short cover / exits (parked)** — no `lock_plus` and no percent stop on shorts (`stop_loss_pct` omitted; optional catastrophic stop is off). Cover when **EMA(9) crosses above SMA(20)** (`action.exit: ma_cross`) and flatten at the **next bar open**, or `session_flatten`. Longs have no take-profit: stop / `lock_stop` / `session_flatten`.
...
YAML fields (long): `stop_mode: lock_plus`, `stop_loss_pct: 1.0`, `lock_trigger_pct: 1.0`, `lock_stop_pct: 1.0`. Parked shorts omit those and set `exit: ma_cross`. Paper / `dry_run` defaults; no live. 60-minute wall-clock cooldown.
...
`config/ema9_trend_risk.example.yaml` is the same 15m noon **long-only** lock-+1% on **AAPL / MSFT only** and sizes each entry to risk ~1% of current equity at the 1.0% initial fill stop (`size.type: risk_pct`). Both names may be open at once when cash covers the second notional; otherwise the later signal is skipped. Parked short sibling: `config/ema9_trend_risk_short_optional.example.yaml`. `--start` / `--end` bound the trade window (prior bars stay for SMA/EMA warmup):
