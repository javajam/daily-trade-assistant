...
3. **Stop** — `stop_mode: lock_plus` (default). Long: initial **fill × 0.99**; first touch of **fill × 1.01** (bar high ≥ that print) locks the stop there. Short: initial **fill × 1.01**; first touch of **fill × 0.99** (−1%, bar low ≤ that print) locks the stop there. The locked stop is live from the **next** bar; a same-bar pullback after the tag still uses the initial 1% stop. A later hit is `lock_stop`.
4. **Cover / exits** — longs have no take-profit (`take_profit_pct` omitted): stop / `lock_stop` / `session_flatten`. Shorts also cover when **EMA(9) crosses above SMA(20)** (`action.exit: ma_cross`) and flatten at the **next bar open**. Session flatten and lock_plus still apply on shorts.
...
YAML fields: `stop_mode: lock_plus`, `stop_loss_pct: 1.0`, `lock_trigger_pct: 1.0`, `lock_stop_pct: 1.0`. Paper / `dry_run` defaults; no live. 60-minute wall-clock cooldown.
