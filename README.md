...
`config/ema9_trend.example.yaml` is the **noon day-trade** book on **AAPL / MSFT** (not TSLA/MU, SPY/QQQ, or NVDA/AMD). **Long-only.** After the range>last-3 vs MA-cross head-to-head, the locked exit is **bar range (high − low) strictly greater than max of the previous 3 bars** (`action.exit: range_expansion`). Fill at that close. Not armed on the entry bar. No MA-cross, no `lock_plus`, no percent stop, no half-take, no pyramid, no volume filter. Writeup: `artifacts/ema9_range3_adopted.md`. Session gates: `entry_cutoff` 12:00 ET, `flatten_by` 15:55 ET. SOXL is optional (`config/ema9_trend_bracket_soxl.example.yaml`, `config/ema9_trend_risk_soxl.example.yaml`). Long-only SPY/QQQ (still lock-+1%): `config/ema9_trend_spy_qqq.example.yaml` / `config/ema9_trend_risk_spy_qqq.example.yaml`. Long-only NVDA/AMD (still lock-+1%): `config/ema9_trend_nvda_amd.example.yaml` / `config/ema9_trend_risk_nvda_amd.example.yaml`. Prior lock-+1% AAPL+MSFT comparison: `config/ema9_trend_bracket_nobe_lock1.example.yaml` / `config/ema9_trend_risk_nobe_lock1.example.yaml`.
...
2. **Exit** — after the fill bar, on each completed bar, leave when that bar’s **range (high − low) > max of the previous 3** (`action.exit: range_expansion`). Fill at **that bar’s close**. Not armed on the entry bar. No live percent stop. `session_flatten` at 15:55 still applies; if the expansion bar is also the flatten bar, `range_expansion` at that close wins.
...
YAML fields: `exit: range_expansion`, `exit_range_bars: 3`. No `stop_loss_pct` / `lock_plus` / `ma_cross_close`. Paper / `dry_run` defaults; no live. 60-minute wall-clock cooldown.
...
`config/ema9_trend_risk.example.yaml` is the same 15m noon long-only range-expansion book on **AAPL / MSFT only** and sizes each entry to risk ~1% of current equity at a 1.0% **reference R** (`size.type: risk_pct`, `stop_pct: 1.0`; no live percent stop). Both names may be open at once when cash covers the second notional; otherwise the later signal is skipped. `--start` / `--end` bound the trade window (prior bars stay for SMA/EMA warmup):
...
Copy to `config/ema9_trend.yaml` or `config/ema9_trend_5m.yaml` (gitignored) to paper the default long-only range-expansion book. Long-only SPY/QQQ (still lock-+1%):
