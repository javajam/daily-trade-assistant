...
2. **Short entry (mirror)** — price crosses **below EMA(9)** AND close **< SMA(20)** AND **RSI(14) > 30**. Same fill (next bar open).
...
4. **No take-profit** — `take_profit_pct` is omitted. Exits are stop / `lock_stop` / `session_flatten` (flatten covers longs and shorts).
