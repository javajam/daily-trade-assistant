...
rules:
  - id: unique-name
    ...
    action:
      ...
      exit: ema_invalid | ma_cross | ma_cross_close | lower_high | fixed_bracket   # default fixed_bracket
...
**MA cross:** `ema_cross` / `sma_cross` with `direction: bullish` or `bearish`. Bullish = previous close ≤ previous MA and current close > current MA (each MA is computed through that bar). **EMA vs SMA:** `ema_sma_cross` with `ema_period` / `sma_period` (bullish = previous EMA ≤ previous SMA and current EMA > current SMA). **MA slope:** `sma_slope` / `ema_slope` with `compare: flat_or_rising` (default; curr MA ≥ prev MA), `rising`, `falling`, `flat_or_falling`, or `flat`. Level compares (`sma` / `ema` + `compare: above|below`) still mean “close vs the current MA only.”
...
`config/ema9_trend.example.yaml` is the **noon day-trade** book on **AAPL / MSFT** (not TSLA/MU, SPY/QQQ, or NVDA/AMD). **Long-only.** After the lock-+1% vs MA-cross vs combo head-to-heads, the locked exit is **EMA(9) crosses below SMA(20) at that bar’s close** (`action.exit: ma_cross_close`). No `lock_plus`, no percent stop, no half-take, no pyramid, no volume filter. Writeup: `artifacts/ema9_macross_adopted.md`. Session gates: `entry_cutoff` 12:00 ET, `flatten_by` 15:55 ET. SOXL is optional (`config/ema9_trend_bracket_soxl.example.yaml`, `config/ema9_trend_risk_soxl.example.yaml`). Long-only SPY/QQQ (still lock-+1%): `config/ema9_trend_spy_qqq.example.yaml` / `config/ema9_trend_risk_spy_qqq.example.yaml`. Long-only NVDA/AMD (still lock-+1%): `config/ema9_trend_nvda_amd.example.yaml` / `config/ema9_trend_risk_nvda_amd.example.yaml`. Prior lock-+1% AAPL+MSFT comparison: `config/ema9_trend_bracket_nobe_lock1.example.yaml` / `config/ema9_trend_risk_nobe_lock1.example.yaml`.
...
2. **Exit** — after entry, on each completed bar, leave when **EMA(9) crosses below SMA(20)** close-to-close (`action.exit: ma_cross_close`). Fill at **that bar’s close**. No live percent stop. `session_flatten` at 15:55 still applies; if the cross bar is also the flatten bar, `ma_cross` at that close wins.
...
YAML fields: `exit: ma_cross_close`, `exit_ema_period: 9`, `exit_sma_period: 20`. No `stop_loss_pct` / `lock_plus`. Paper / `dry_run` defaults; no live. 60-minute wall-clock cooldown.
...
python -m dta_bot backtest --config config/ema9_trend.example.yaml --source yahoo \
  --compare-config config/ema9_trend_bracket_nobe_lock1.example.yaml --combined-only \
  --output artifacts/ema9_lock1_macross.json --report artifacts/ema9_lock1_macross.md
```

Copy to `config/ema9_trend.yaml` or `config/ema9_trend_5m.yaml` (gitignored) to paper the default long-only MA-cross-at-close book. Long-only SPY/QQQ (still lock-+1%):
