...
rules:
  - id: unique-name
    ...
    action:
      ...
      exit: ema_invalid | ma_cross | ma_cross_close | lower_high | range_expansion | fixed_bracket
      exit_ema_period: 9                  # ema_invalid / ma_cross / ma_cross_close (alias: ema_period)
      exit_sma_period: 20                 # ma_cross / ma_cross_close (alias: sma_period)
      exit_range_bars: 3                  # range_expansion: compare vs previous N bars (high − low)
      ...
      partial_take_on_lock: false  # lock_plus only; sell half at +lock, lock remainder
      partial_take_be: false      # lock_plus only; sell half at +lock, BE remainder at fill
      take_anchor: signal    # signal = take from signal close; entry = take from fill
                             # forced to entry when pyramid_on_lock is true
      take_profit_pct: 3.0   # ignored when exit is ema_invalid, ma_cross, ma_cross_close, or lower_high; omit for stop-only
...
`config/ema9_trend.example.yaml` is the **noon day-trade** book on **AAPL / MSFT** (not TSLA/MU, SPY/QQQ, or NVDA/AMD). **Long-only.** After the range>last-3 + hard 1% fill-stop vs pure range3 head-to-head, the locked exit is **bar range (high − low) strictly greater than max of the previous 3 bars** (`action.exit: range_expansion`) **or** a hard 1% stop at fill × 0.99 (`stop_mode: entry_pct`), whichever first. Range fill at that close. Not armed on the entry bar. The stop never moves — not `lock_plus`. No MA-cross, no half-take, no pyramid, no volume filter. Writeup: `artifacts/ema9_range3_fixed1_adopted.md`. Session gates: `entry_cutoff` 12:00 ET, `flatten_by` 15:55 ET. SOXL is optional (`config/ema9_trend_bracket_soxl.example.yaml`, `config/ema9_trend_risk_soxl.example.yaml`). Long-only SPY/QQQ (still lock-+1%): `config/ema9_trend_spy_qqq.example.yaml` / `config/ema9_trend_risk_spy_qqq.example.yaml`. Long-only NVDA/AMD (still lock-+1%): `config/ema9_trend_nvda_amd.example.yaml` / `config/ema9_trend_risk_nvda_amd.example.yaml`. Prior lock-+1% AAPL+MSFT comparison: `config/ema9_trend_bracket_nobe_lock1.example.yaml` / `config/ema9_trend_risk_nobe_lock1.example.yaml`.
...
2. **Exit** — after the fill bar, on each completed bar, leave when that bar’s **range (high − low) > max of the previous 3** (`action.exit: range_expansion`) **or** when the bar low tags **fill × 0.99** (`stop_mode: entry_pct`), whichever first. Range fill at **that bar’s close**. Not armed on the entry bar. The stop never trails or locks at +1%. Same-bar stop + range → **stop**. `session_flatten` at 15:55 still applies; if the expansion bar is also the flatten bar and the stop did not hit, `range_expansion` at that close wins.
3. **No short rule** in the default book. Cover-only shorts remain a recorded experiment (`artifacts/ema9_long_short_15m.md`).
4. **Break-even is off** (`breakeven_after_bars` omitted / 0).
5. **Optional / comparison siblings** (same entry / session): **lock-+1%** (prior default, kept for comparison): `config/ema9_trend_bracket_nobe_lock1.example.yaml` / `config/ema9_trend_risk_nobe_lock1.example.yaml`. Fixed 1% from the fill (`stop_mode: entry_pct`, `config/ema9_trend_bracket_nobe_fixed1.example.yaml`); trail 1% from peak since entry (`stop_mode: trail`, `config/ema9_trend_bracket_nobe_trail1.example.yaml`). Matching 1% risk YAMLs: `config/ema9_trend_risk_nobe_fixed1.example.yaml`, `config/ema9_trend_risk_nobe_trail1.example.yaml`. **Lock-+1% + half-take at +1%** (no add: sell floor(half) at original fill × 1.01, lock the remainder there): `config/ema9_trend_bracket_nobe_lock1_half.example.yaml` / `config/ema9_trend_risk_nobe_lock1_half.example.yaml`. Same Yahoo 15m AAPL+MSFT tape: 10-share **51 trades, 60.78%, $341.01**, max DD $127.57 vs lock-+1% **51 / 60.78% / $301.49** — all 20 lock-arm events sold half at the +1% trigger (0 gap-open scale-outs; 0 size-1 skips). The $39.52 lift is 8 `lock_stop`s where the remainder gapped through the lock. **Beats** lock-+1% on that 10-share window. August 2026 1% risk **14 / 71.43% / $4,799.30** vs **15 / 73.33% / $5,489.78** — does **not** beat (B’s higher equity after half-takes sized +1 share on AAPL 2026-08-26 and the next-open fill cash-skipped; that miss was A’s +$871.18 `lock_stop`). Writeup: `artifacts/ema9_lock1_half.md`. **Lock-+1% + half-take at +1% + BE remainder** (sell floor(half) at fill × 1.01, rest the remainder stop at fill × 1.00 — not lock at +1%): `config/ema9_trend_bracket_nobe_lock1_half_be.example.yaml` / `config/ema9_trend_risk_nobe_lock1_half_be.example.yaml`. Same Yahoo 15m AAPL+MSFT tape: 10-share **51 trades, 60.78%, $340.52**, max DD $147.09 vs lock-+1% **51 / 60.78% / $301.49** and vs half+lock **51 / 60.78% / $341.01**. **Beats** lock-+1% on that 10-share window; **$0.49 behind** half+lock (20 half-takes at the trigger; 5 `breakeven_stop` / 15 flatten on the remainder). August 2026 1% risk **12 / 75.00% / $3,872.16** vs **15 / 73.33% / $5,489.78** — does **not** beat. Writeup: `artifacts/ema9_lock1_half_be.md`. **Lock-+1% + add-at-+0.5%** (no take: add the same share count at original fill × 1.005, then lock at +1% on the full lot): `config/ema9_trend_bracket_nobe_lock1_add05.example.yaml` / `config/ema9_trend_risk_nobe_lock1_add05.example.yaml`. A bar that gaps through both prints adds first, then locks. Same Yahoo 15m AAPL+MSFT tape: 10-share **51 trades, 54.90%, $265.32**, max DD $220.96 vs lock-+1% **51 / 60.78% / $301.49** — 35 added at +0.5% (0 cash skips), 20 then locked, 15 added and never locked. Does **not** beat lock-+1%. August 2026 1% risk is **identical** ($5,489.78; 13/13 add cash-skips). Writeup: `artifacts/ema9_lock1_add05.md`. **Lock-+1% + pyramid + 2% fill take** (add the same share count when the +1% lock arms; stop stays at original fill × 1.01 on the full lot; take the entire position at original fill × 1.02; no volume filter): `config/ema9_trend_bracket_nobe_lock1_pyramid2.example.yaml` / `config/ema9_trend_risk_nobe_lock1_pyramid2.example.yaml`. Add fill: open if the bar gaps through fill × 1.01, else the trigger. Cash for the add is not reserved at entry. Live does not auto-add. Same Yahoo 15m AAPL+MSFT tape: 10-share **51 trades, 58.82%, $284.79**, max DD $137.13 vs lock-+1% **51 / 60.78% / $301.49** — all 20 lock-arm events added (0 cash skips); only 1 `take_2pct`. Pyramid+2% does **not** beat lock-+1% on that window. August 2026 1% risk **15 / 73.33% / $6,630.33** vs **15 / 73.33% / $5,489.78**, but **0 of 6 adds filled** (cash skip; the extra is the 2% take on the original lot). Writeup: `artifacts/ema9_lock1_pyramid2.md`. **Lock-+1% + vol>prev only** (same lock exit; signal volume > previous bar): `config/ema9_trend_bracket_nobe_lock1_vol.example.yaml` / `config/ema9_trend_risk_nobe_lock1_vol.example.yaml`. Same tape 10-share **33 trades, 60.61%, $182.91**, max DD $183.54 vs lock-+1% **51 / 60.78% / $301.49** — volume does **not** improve lock-+1% (the 21 filtered fills were net +$152.04). August 1% risk **11 / 72.73% / $3,613.53** vs **15 / 73.33% / $5,489.78**. Writeup: `artifacts/ema9_lock1_vol.md`. **Lower-high + vol>prev** (no lock-+1% / percent stop; signal volume > previous bar; exit at the completed bar close on a lower high): `config/ema9_trend_bracket_nobe_lh_vol.example.yaml` / `config/ema9_trend_risk_nobe_lh_vol.example.yaml`. Risk sizing uses `stop_pct: 1.0` as a reference R only (1% of entry). Same Yahoo 15m AAPL+MSFT tape: 10-share **34 trades, 55.88%, $130.91**, max DD $179.48 (all 34 `lower_high`; 0 flatten) vs lock-+1% **51 / 60.78% / $301.49**. August 2026 1% risk **12 trades, 58.33%, $1,059.41**, max DD $881.79 vs lock-+1% **15 / 73.33% / $5,489.78**. Writeup: `artifacts/ema9_lh_vol.md`. **EMA9×below SMA20 at close (prior locked exit, kept for comparison)** (no lock-+1% / no % stop / no volume filter / no half-take / no pyramid; same noon entry; fill at the completed bar close on a close-to-close EMA vs SMA pair-cross — not the next-open `ma_cross` cover): `config/ema9_trend_bracket_nobe_macross.example.yaml` / `config/ema9_trend_risk_nobe_macross.example.yaml`. Risk sizing uses `stop_pct: 1.0` as a reference R only (1% of entry). If the cross bar is also the flatten bar, `ma_cross` at that close wins. Same Yahoo 15m AAPL+MSFT tape: 10-share **50 trades, 56.00%, $427.81**, max DD $147.68 (37 `ma_cross` / 13 flatten) vs lock-+1% **51 / 60.78% / $301.49**. **Beats** lock-+1% on that 10-share window ($126.32 ahead) with a lower win rate. August 2026 1% risk **14 / 57.14% / $2,998.96**, max DD $2,510.94 vs **15 / 73.33% / $5,489.78** — does **not** beat (missed AAPL 2026-08-04 A `lock_stop` +$1,007.60 on a cash skip). Chosen as the locked day-trade exit after those head-to-heads; later replaced by range>last-3. Writeup: `artifacts/ema9_macross_adopted.md`, `artifacts/ema9_lock1_macross.md`. **Lock-+1% + EMA9×below SMA20 at close** (both exits live; first hit wins; no half-take / no pyramid / no volume filter): `config/ema9_trend_bracket_nobe_lock1_macross.example.yaml` / `config/ema9_trend_risk_nobe_lock1_macross.example.yaml`. Same-bar stop / `lock_stop` beats the pair-cross. Same Yahoo 15m AAPL+MSFT tape: 10-share **51 trades, 54.90%, $296.62**, max DD $106.37 vs lock-+1% **51 / 60.78% / $301.49** and vs pure MA-cross **50 / 56.00% / $427.81**. Does **not** beat either on that 10-share window ($4.87 behind lock; $131.19 behind pure MA-cross). August 2026 1% risk **15 / 60.00% / $4,586.73**, max DD $1,915.59 vs lock **15 / 73.33% / $5,489.78** (does **not** beat) and vs pure MA-cross **14 / 57.14% / $2,998.96** (**beats** B; keeps A’s `lock_stop`s). Writeup: `artifacts/ema9_lock1_plus_macross.md`. **Range > last 3 bars (prior locked exit, kept for comparison)** (same noon entry/gates; `action.exit: range_expansion` — current completed bar range (high − low) strictly greater than max of the previous 3; fill at that close; not on the entry bar; no percent stop): `config/ema9_trend_bracket_nobe_range3.example.yaml` / `config/ema9_trend_risk_nobe_range3.example.yaml`. Same Yahoo 15m tape (2026-06-25→2026-09-18): 10-share **54 trades, 75.93%, $696.66**, max DD $115.38 vs then-locked MA-cross **50 / 58.00% / $560.31**. **Beats** ($136.35 ahead). August 2026 1% risk **16 / 87.50% / $6,008.19**, max DD $2,028.83 vs then-locked **14 / 57.14% / $2,998.96**. **Beats**. Writeup: `artifacts/ema9_range3.md`, `artifacts/ema9_range3_adopted.md`. **Range > last 3 + hard 1% fill stop (now the default)** (same range exit; `stop_mode: entry_pct` at fill × 0.99; never moves / not `lock_plus`; stop OR range OR flatten, whichever first; same-bar stop beats range): `config/ema9_trend.example.yaml` / `config/ema9_trend_risk.example.yaml` (named copies: `config/ema9_trend_bracket_nobe_range3_fixed1.example.yaml` / `config/ema9_trend_risk_nobe_range3_fixed1.example.yaml`). Same Yahoo 15m tape: 10-share **55 / 74.55% / $682.32**, max DD $99.05 vs then-locked pure range3 **54 / 75.93% / $696.66**. Does **not** beat ($14.34 behind); only slightly costs, with a smaller DD. August 2026 1% risk is **identical** (**16 / 87.50% / $6,008.19**; 0 stops). Chosen as the locked day-trade book after that head-to-head (protection priority). Writeup: `artifacts/ema9_range3_fixed1.md`, `artifacts/ema9_range3_fixed1_adopted.md`.
...
YAML fields: `exit: range_expansion`, `exit_range_bars: 3`, `stop_mode: entry_pct`, `stop_loss_pct: 1.0`. No `lock_plus` / `ma_cross_close`. Paper / `dry_run` defaults; no live. 60-minute wall-clock cooldown.
...
- `flatten_by: "15:55"` — force-flat at the close of the bar that contains 3:55 PM ET. On **15m** RTH bars opening `:00,:15,:30,:45` that is the **15:45 ET bar close** (last regular 15m bar before 16:00, labeled as the end-of-day flatten aligned with “by 15:55”). On **5m** that is the **15:50 ET bar close** (last 5m bar that completes at/before 15:55). Stop / EMA-invalid / lower-high / `range_expansion` / `ma_cross_close` / a same-bar MA-cross signal still win if they hit first; `ma_cross` fills at the next open, so a flatten-bar close is `session_flatten`. `range_expansion` and `ma_cross_close` fill at that close, so the signal exit wins over flatten. Exit reason: `session_flatten`.
- Overnight control: `config/ema9_trend_bracket_overnight.example.yaml`.

`config/ema9_trend_risk.example.yaml` is the same 15m noon long-only range-expansion + hard 1% fill-stop book on **AAPL / MSFT only** and sizes each entry to risk ~1% of current equity at that live 1.0% stop (`size.type: risk_pct`, `stop_pct: 1.0`, `stop_mode: entry_pct`). Both names may be open at once when cash covers the second notional; otherwise the later signal is skipped. `--start` / `--end` bound the trade window (prior bars stay for SMA/EMA warmup):
...
python -m dta_bot backtest --config config/ema9_trend.example.yaml --source yahoo \
  --compare-config config/ema9_trend_bracket_nobe_macross.example.yaml --combined-only \
